...
export default function Map() {
  ...
  useEffect(() => {
    ...
    map.current.on('load', function () {
      ...
      document.addEventListener('click', (event) => {
        if (!searchInput.contains(event.target) && !searchResults.contains(event.target) && document.activeElement !== searchInput) {
  ...
  async function fetchMapRecords() {
    ...
    const waterBodyTypes = Array.from(document.querySelectorAll('input[type="checkbox"].body_filter:checked'))
      .map((checkbox) => checkbox.nextElementSibling.textContent).join(",");
    
    // Get all checked source types
    const sourceTypes = Array.from(document.querySelectorAll('input[type="checkbox"].source_filter:checked'))
    .map((checkbox) => checkbox.nextElementSibling.textContent).join(",");
    ...
    const response = await axios.get('http://localhost:8080/maprecordsbyparams', { params })
  ...
  function populateSearchResults() {
    ...
    searchPlace(searchInput.value).then((results) => {
      ...
      const searchResultItems = document.getElementById('search_results').getElementsByTagName('a');
      for (var i = 0; i < searchResultItems.length; i++) {
        let item = searchResultItems[i];
        item.addEventListener('click', () => {
          console.log(searchResultItems)
          console.log(item.dataset.bbox)
          map.current.fitBounds(item.dataset.bbox.split(',').map((value) => parseFloat(value)), {maxZoom: 19});
          searchResults.style.display = 'none';
  ...
  async function addTileLayers() {
    ...
    records.forEach((record) => {
      ...
      map.current.on('click', 'sample_'+layerId, function (e) {
        ...
        .setLngLat(e.lngLat)
        .setHTML( "<b>Sample ID:</b> " + record.mapRecord_ID + "<br/>" +
  ...
  async function searchPlace(place) {
    const response = await fetch(`https://api.maptiler.com/geocoding/${place}.json?country=gb&key=${API_KEY}`);
  ...
  return (
    <div>
      ...
      <button id="test_button" onClick={addTileLayers}>Load Tiles</button>
      ...
      <div className="data_filters_wrap">
        ...
        <div id="source_filters" className="source_filters white-box">
          ...
          <div className="source_checkboxes">
            ...
            <label for="user_testkit_data">User Test-kit Data</label>
          </div>
        </div>
